...
import { EntryPoint, SimpleAccount, VerifyingPaymaster, VerifyingPaymaster__factory } from "../typechain";
...
describe("EntryPoint with VerifyingPaymaster", function () {
  ...
  describe("#parsePaymasterAndData", () => {
    it("should parse data properly", async () => {
      const paymasterAndData = packPaymasterData(
        ...
        hexConcat([defaultAbiCoder.encode(["uint48", "uint48"], [MOCK_VALID_UNTIL, MOCK_VALID_AFTER]), MOCK_SIG]),
      );
  ...
  describe("#validatePaymasterUserOp", () => {
    it("should reject on no signature", async () => {
      ...
      expect(await simulateValidation(userOp, entryPoint.address).catch((e) => decodeRevertReason(e))).to.include(
    ...
    describe("with wrong signature", () => {
      ...
      it("handleOp revert on signature failure in handleOps", async () => {
        await expect(entryPoint.estimateGas.handleOps([wrongSigUserOp], beneficiaryAddress)).to.revertedWith(
  ...
  describe("#signerAccessControl", () => {
    let newSigner: Wallet;
    let userOp: PackedUserOperation;

    before(async () => {
      newSigner = createAccountOwner();
    ...
    it("should allow owner to change verifier", async () => {
      await expect(paymaster.setVerifier(newSigner.address))
        .to.emit(paymaster, "VerifierChanged")
        .withArgs(newSigner.address);

      expect(await paymaster.verifier()).to.equal(newSigner.address);
    });

    it("should not allow non-owner to change verifier", async () => {
      const nonOwner = createAccountOwner();
      await expect(paymaster.connect(nonOwner).setVerifier(nonOwner.address)).to.be.reverted;
    });

    it("should accept signature from new verifier", async () => {
      const hash = await paymaster.getHash(packUserOp(userOp), MOCK_VALID_UNTIL, MOCK_VALID_AFTER);
      const sig = await newSigner.signMessage(arrayify(hash));
      const newUserOp = await fillSignAndPack(
        {
          ...userOp,
          paymaster: paymaster.address,
          paymasterData: hexConcat([
      ...
      const res = await simulateValidation(newUserOp, entryPoint.address);
      const validationData = parseValidationData(res.returnInfo.paymasterValidationData);
      expect(validationData.aggregator).to.equal(AddressZero);
    });

    it("should reject signature from old verifier", async () => {
      const hash = await paymaster.getHash(packUserOp(userOp), MOCK_VALID_UNTIL, MOCK_VALID_AFTER);
      const sig = await offchainSigner.signMessage(arrayify(hash));
      const oldSignerUserOp = await fillSignAndPack(
        {
          ...userOp,
          paymaster: paymaster.address,
          paymasterData: hexConcat([
      ...
      const res = await simulateValidation(oldSignerUserOp, entryPoint.address);
      const validationData = parseValidationData(res.returnInfo.paymasterValidationData);
      expect(validationData.aggregator).to.match(/0x0*1$/);
    });
  });
});
